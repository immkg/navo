    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          <button onClick={() => setIsModalOpen(true)} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg shadow-sm hover:bg-blue-700 font-medium transition">
            + New Intent
          </button>
          <Link to="/planner" className="bg-gray-900 text-white px-5 py-2.5 rounded-lg shadow-sm hover:bg-gray-800 font-medium transition">
            Go to Planner &rarr;
          </Link>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-gray-500">Loading intents...</div>
      ) : intents.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {intents.map(intent => (
            ...
            >
              <h2 className="text-xl font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">{intent.title}</h2>
              {intent.description && (
                <p className="text-gray-500 mt-2 text-sm line-clamp-2">{intent.description}</p>
              )}
              <div className="mt-4 pt-4 border-t flex justify-between items-center">
                <span className="text-xs font-medium px-2.5 py-1 bg-green-100 text-green-800 rounded-full capitalize">
                  {intent.status}
                </span>
                <span className="text-sm text-gray-400 group-hover:text-blue-500 transition-colors">View Graph &rarr;</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6">
            ...
            <form onSubmit={handleCreateIntent}>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">What is your intent?</label>
                <input 
                  type="text" 
                  autoFocus
                  required
